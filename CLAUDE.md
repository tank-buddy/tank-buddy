...
- **The plugins that write are only loaded when publishing**, selected by `SEMANTIC_RELEASE_PUBLISH` in `release.config.mjs`. This is not tidiness: `@semantic-release/git` verifies its access with a real `git push --dry-run` during `verifyConditions` — before any commit is analysed, and **regardless of semantic-release's own `--dry-run`**. A `contents: read` job therefore dies with `EGITNOPERMISSION` and never works out a version at all, which is exactly how the first automated run failed. `@semantic-release/github` checks its token the same way. Working out a version number needs neither plugin, so the read-only job stays read-only. The config is `.mjs` rather than JSON because that condition — and the reasoning here — cannot be expressed in JSON.
